...
__global__ void
hausdorffDistanceCPUSync(const bool *img1, const bool *img2, bool *img1P, bool *img2P, bool *img1PAux, bool *img2PAux, int *grownEnough,
const int WIDTH, const int HEIGHT, const bool* structElement, const char STRUCT_SIZE)
{
	//const int id = blockDim.x * blockIdx.x + threadIdx.x;
	const int id = blockDim.x * blockIdx.x + threadIdx.x;
	const int y = id / WIDTH, x = id % WIDTH;
	...
	if (id < WIDTH*HEIGHT){
		...
		if (img1PAux[id]){
			if (x + 1 < WIDTH) img1P[id + 1] = true;
			if (x - 1 >= 0) img1P[id - 1] = true;
			if (y + 1 < HEIGHT)img1P[id + WIDTH] = true;
			if (y - 1 >= 0) img1P[id - WIDTH] = true;
			//diagonais
			/*if (y - 1 >= 0 && x + 1 < HEIGHT) img1P[id - WIDTH + 1] = true;
			if (y - 1 >= 0 && x - 1 >= 0) img1P[id - WIDTH - 1] = true;
			if (y + 1 < HEIGHT && x + 1 < WIDTH) img1P[id + WIDTH + 1] = true;
			if (y + 1 < HEIGHT && x - 1 >= 0) img1P[id + WIDTH - 1] = true;*/
		}
		if (img2PAux[id]){
			if (x + 1 < WIDTH) img2P[id + 1] = true;
			if (x - 1 >= 0) img2P[id - 1] = true;
			if (y + 1 < HEIGHT)img2P[id + WIDTH] = true;
			if (y - 1 >= 0) img2P[id - WIDTH] = true;
			//diagonais
			/*if (y - 1 >= 0 && x + 1 < HEIGHT) img2P[id - WIDTH + 1] = true;
			if (y - 1 >= 0 && x - 1 >= 0) img2P[id - WIDTH - 1] = true;
			if (y + 1 < HEIGHT && x + 1 < WIDTH) img2P[id + WIDTH + 1] = true;
			if (y + 1 < HEIGHT && x - 1 >= 0) img2P[id + WIDTH - 1] = true;*/
		}
...
int
hdCPUSync(bool *img1, bool *img2, const int WIDTH, const int HEIGHT, bool *structElement, const char STRUCT_SIZE)
{
	...
	//while (distance < 1){
		...
		hausdorffDistanceCPUSync << <blocksPerGrid, threadsPerBlock>> >
			(d_img1, d_img2, d_img1P, d_img2P, d_img1PAux, d_img2PAux, d_grownEnough, WIDTH, HEIGHT, d_structElement, STRUCT_SIZE);
		...
		err = cudaMemcpy(d_img1PAux, d_img1P, size, cudaMemcpyDeviceToDevice);
		...
		err = cudaMemcpy(d_img2PAux, d_img2P, size, cudaMemcpyDeviceToDevice);
		...
		err = cudaMemcpy(&h_grownEnough, d_grownEnough, sizeof(int), cudaMemcpyDeviceToHost);
		if (err != cudaSuccess)
		{
