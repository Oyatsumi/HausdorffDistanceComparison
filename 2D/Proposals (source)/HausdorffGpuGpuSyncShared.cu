...
__device__ void blockSync2_shared(int goalVal){
	//int tId = threadIdx.x;
	//if (tId == 0){
	atomicAdd(&g_mutex, 1);
...
__device__ void blockSync3_shared(int goalVal){
	//int tId = threadIdx.x;
	//if (tId == 0){
...
__device__ void
getPosition(int currentId, const int WIDTH, const int TILE_SIZE){
...
__global__ void
hausdorffDistanceGPUSyncShared(const bool *img1, const bool *img2, bool *img1P, bool *img2P, bool *img1PAux, bool *img2PAux,
const int WIDTH, const int HEIGHT, const int TILE_SIZE, const bool* structElement, const char STRUCT_SIZE, int *d_distance)
{
	extern __shared__ bool imgsBuffer[];
	//if (threadIdx.x == 0) imgsBuffer[0] = 0;
	bool* img1Buffer = &imgsBuffer[0];
	bool* img2Buffer = &imgsBuffer[TILE_SIZE+ (WIDTH*2+2)];
	...
	while (!grownEnough || dist == 0 || g_mutex != blockDim.x*gridDim.x /*esse ultimo foi extremamente necessario*/){
		...
		g_mutex = 0;
		blockSync3_shared(blockDim.x*gridDim.x);
		//printf("%d", TILE_SIZE);
		for (int k = 0; k < TILE_SIZE; k++){//for tilesize
			currentId = id*TILE_SIZE + k;
		...
		g_mutex2 = 0;
		blockSync2_shared(blockDim.x*gridDim.x);
...
int
hdGPUSyncShared(bool *img1, bool *img2, const int WIDTH, const int HEIGHT, bool *structElement, const int STRUCT_SIZE)
{
	...
	err = cudaMalloc((void **)&d_img1PAux, WIDTH*HEIGHT*sizeof(volatile bool));
	...
	err = cudaMalloc((void **)&d_img2PAux, WIDTH*HEIGHT*sizeof(volatile bool));
	...
	hausdorffDistanceGPUSyncShared << <blocksPerGrid, threadsPerBlock, 12287 * 32/8 * sizeof(bool) >> >
