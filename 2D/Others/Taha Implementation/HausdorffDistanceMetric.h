...
public:
	...
	double calc0(ImageType *image1, ImageType *image2, std::vector<double> *distances){
		...
		while (!movingIt.IsAtEnd() && !fixedIt.IsAtEnd()){
			if (movingIt.Get()>thd && fixedIt.Get() <= thd){
				trueVoxels_2[FN_index].value = movingIt.Get();
				trueVoxels_2[FN_index].x = movingIt.GetIndex()[0];
				trueVoxels_2[FN_index].y = movingIt.GetIndex()[1];
				trueVoxels_2[FN_index].z = movingIt.GetIndex()[2];
				FN_index++;
			}
			if (fixedIt.Get()>thd){
				retrievedVoxels_2[FP_index].value = fixedIt.Get();
				retrievedVoxels_2[FP_index].x = fixedIt.GetIndex()[0];
				retrievedVoxels_2[FP_index].y = fixedIt.GetIndex()[1];
				retrievedVoxels_2[FP_index].z = fixedIt.GetIndex()[2];
				FP_index++;
			}
			++movingIt;
			++fixedIt;
		...
		while (index_1 < numberTrue_1 || index_2 < numberTrue_2){
			...
			if (index_1 < numberTrue_1){
				...
				for (int x = 0; x < numberRetr_1; x++){
					...
					//sup
					distI = std::max(std::abs((testpoint.x - p.x)), std::abs((testpoint.y - p.y)));
					//distI = std::max(distI, (testpoint.z - p.z));
					dist = distI;
					//sup
	...
	double s1(double s01, double max, int maxMax, VoxelInfo p, VoxelInfo *comparretriecedVoxels, int step){
		...
		for (int j = 0; j< maxMax && s01 > max; j = j + step){
			...
			//double dist = std::sqrt((double)((testpoint.x - p.x)*(testpoint.x - p.x) + (testpoint.y - p.y)*(testpoint.y - p.y) + (testpoint.z - p.z)*(testpoint.z - p.z)));
			//sup
			distI = std::max((testpoint.x - p.x), (testpoint.y - p.y));
			//distI = std::max(distI, (testpoint.z - p.z));
			dist = distI;
			//sup
			x++;
